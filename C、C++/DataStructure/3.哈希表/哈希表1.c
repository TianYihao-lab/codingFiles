...
//  ----------------
// 定义哈希表结构体
typedef struct Element{     // 定义哈希表中的元素
	int key;
}Element,*e;
...
void init(hashTable _hash){     // 初始化哈希表
...
void insert(hashTable _hash,e _element){        // 插入元素
	int cmp = Hash(_element->key);
	_hash->table[cmp] = _element;
}

bool find(hashTable _hash,int _key){            // 哈希表的查找
	int cmp = Hash(_key);
...
/*
为什么会用到多次结构体指针，并且存在结构体指针作为结构体成员？
