...
// 在数组list的第index个位置上插入一个元素element
void insertList(ArryList list, E element, int index) {  
	// 碰到插入位置不对
...
// 删除指定位置上的元素
void deleteList(ArryList list, int index) {        
...
// 获得指定位置上的元素
E* getList(ArryList list, int index) {  
...
// 寻找元素e在数组中的位序
int findList(ArryList list,E e) {   
	for (int i = 0;i < list->length;i++) {
